...
const handleSubmit = (entry: LogBookEntry) => {
  ...
  const endpoint = isNew ? "/api/log-books" : `/api/log-books/${entry.id.replace('temp-', '')}`;
  ...
  function renderField(group: DynamicGroup, field: DynamicField, entry: LogBookEntry) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="w-full">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">Your Logbook Entries</h2>
              ...
              onClick={() => {
                ...
                logBookTemplate.dynamicSchema.groups.forEach(group => {
                  ...
                  group.fields.forEach(field => {
                    const fieldName = field.label.toLowerCase().replace(/\s+/g, "_");
                    if (field.type === "date") {
                      dynamicFields[group.name][fieldName] = new Date().toISOString().split("T")[0];
                    } else {
                      dynamicFields[group.name][fieldName] = field.defaultValue || "";
                    }
                  });
                });
                    ...
                    {logBookTemplate?.dynamicSchema.groups.map(group => (
                      group.fields.map(field => {
                        ...
                        return (
                          ...
                          >
                            {field.label}
                            {sortConfig && sortConfig.key === `${group.name}.${fieldName}` && (
                              <span className="ml-1">
                    ...
                    <TableRow key={entry.id}>
                      ...
                      {logBookTemplate?.dynamicSchema.groups.map(group => (
                        group.fields.map(field => {
                          ...
                          return (
                            <TableCell key={`${entry.id}-${group.name}-${fieldName}`}>
                              {entry.isEditing ? (
                        ...
                        {entry.isEditing ? (
                          ...
                          <div className="flex items-center space-x-2">
                            <Badge variant="outline" className="cursor-pointer" onClick={() => {
                              ...
                              {loading.submit && entry.id === submittingEntryId ? (
