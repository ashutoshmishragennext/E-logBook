...
const Department = () => {
  ...
  const [currentBranch, setCurrentBranch] = useState<{ id: string; name: string; code?: string; description?: string } | null>(null);
  ...
  const fetchBranches = async () => {
    try {
      ...
      setError("Error fetching branches: " + (err instanceof Error ? err.message : "Unknown error"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const openSidebar = (branch: { id: string; name: string; code?: string; description?: string } | null = null) => {
    if (branch) {
  ...
  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    try {
      ...
      setError("Error saving branch: " + (err instanceof Error ? err.message : "Unknown error"));
    } finally {
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Description</th>
            <th scope="col" className="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[110px]">Actions</th>
          </tr>
        </thead>
          ...
          {isLoading && branches.length === 0 ? (
            ...
            filteredBranches.map((branch, index) => (
              <tr key={branch.id} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-3 whitespace-nowrap text-sm font-medium">{branch.name}</td>
                ...
                <td className="px-3 py-3 hidden md:table-cell">
                  <p className="text-sm text-gray-500 line-clamp-2">{branch.description || "—"}</p>
                </td>
  ...
  {/* Responsive Sidebar */}
  <div className={`fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-lg border-l border-gray-200 transform transition-transform duration-300 ease-in-out z-50 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
